                                      ...
                                      float *closest_distances)
{
    ...
    // // 打印前3个点到前3个中心的距离
    // diskann::cout << "dist_matrix 部分内容(前3个点,每点前3个中心):" << std::endl;
    // for (size_t i = 0; i < std::min<size_t>(3, num_points); ++i) {
    //     diskann::cout << "点" << i << ": ";
    //     for (size_t j = 0; j < std::min<size_t>(3, num_centers); ++j) {
    //         diskann::cout << dist_matrix[i * num_centers + j] << " ";
    //     }
    //     diskann::cout << std::endl;
    //     std::this_thread::sleep_for(std::chrono::milliseconds(1)); // 延时1毫秒
    // }
    if (k == 1)
    {
#pragma omp parallel for schedule(static, 8192)
...
#pragma omp parallel for schedule(static, 8192)
        for (int64_t i = 0; i < (int64_t)num_points; i++)
        {
            ...
            for (size_t j = 0; j < k; j++)
            {
                ...
                if (closest_distances != nullptr)
                {
                    closest_distances[i * k + j] = this_piv.piv_dist; // 保存距离
                }
                top_k_queue.pop();
            }
        }
...
void compute_closest_centers(float *data, size_t num_points, size_t dim, float *pivot_data, size_t num_centers,
                             ...
                             float *pts_norms_squared, float *k_closest_distances)
{
    if (k > num_centers)
    {
        diskann::cout << "ERROR: k (" << k << ") > num_center(" << num_centers << ")" << std::endl;
    ...
    float *closest_distances = new float[PAR_BLOCK_SIZE * k];
    ...
    for (size_t cur_blk = 0; cur_blk < N_BLOCKS; cur_blk++)
    {
        ...
        math_utils::compute_closest_centers_in_block(data_cur_blk, num_pts_blk, dim, pivot_data, num_centers,
                                                     ...
                                                     distance_matrix, k, closest_distances);
...
#pragma omp parallel for schedule(static, 1)
             ...
             j < std::min((int64_t)num_points, (int64_t)((cur_blk + 1) * PAR_BLOCK_SIZE)); j++)
        {
            for (size_t l = 0; l < k; l++)
            {
                ...
                if (k_closest_distances != NULL)
                {
                    float this_distance = closest_distances[(j - cur_blk * PAR_BLOCK_SIZE) * k + l];
                    k_closest_distances[j * k + l] = (float)this_distance;
                }
