...
// given training data in train_data of dimensions num_train * dim, generate guildpq
                       ...
                       bool make_zero_mean)
{
    ...
    std::unique_ptr<float[]> prev_layer_pivots = nullptr;
...
#pragma omp parallel for schedule(static, 65536)
        ...
        else
        {
            ...
            for (uint64_t j = 0; j < num_centers; j++)
            {
                std::memcpy(cur_pivot_data.get() + j * cur_chunk_size,
                            prev_layer_pivots.get() + j * cur_chunk_size, cur_chunk_size * sizeof(float));
            }
        }
        ...
        // 保存当前层的聚类中心，用于下一层
        prev_layer_pivots = std::move(cur_pivot_data);
    }
    
    // std::vector<float> global_centers(num_pq_chunks * num_centers * dim, 0.0f);
    // // 将每个 chunk 的聚类中心复制到全局中心,以指导分shard
    // for (size_t i = 0; i < num_pq_chunks; i++) {
    //     size_t cur_chunk_size = chunk_offsets[i + 1] - chunk_offsets[i];
    //     size_t chunk_offset = chunk_offsets[i];

    //     for (size_t j = 0; j < num_centers; j++) {
    //         for (size_t k = 0; k < cur_chunk_size; k++) {
    //             global_centers[j * dim + chunk_offset + k] = full_pivot_data[j * dim + chunk_offset + k];
    //         }
    //     }
    // }
    // 保存 PQ pivots
    ...
    // diskann::save_bin<float>("global_pivot_vec.bin", global_centers.data(), num_centers, dim);
...
int generate_pq_data_from_pivots(const std::string &data_file, uint32_t num_centers, uint32_t num_pq_chunks,
                                 ...
                                 bool use_opq)
{
    // 输出 pq_compressed_vectors_path
    diskann::cout << "PQ Compressed Vectors Path: " << pq_compressed_vectors_path << std::endl;

    // 检查是否已经存在压缩后的 PQ 文件
    if (file_exists(pq_compressed_vectors_path))
    {
        std::ifstream compressed_file_reader(pq_compressed_vectors_path, std::ios::binary);
        if (compressed_file_reader.is_open())
        {
            uint32_t existing_num_points, existing_num_chunks;
            compressed_file_reader.read((char *)&existing_num_points, sizeof(uint32_t));
            compressed_file_reader.read((char *)&existing_num_chunks, sizeof(uint32_t));

            // 如果文件的元数据匹配，则跳过生成
            if (existing_num_chunks == num_pq_chunks)
            {
                diskann::cout << "Compressed PQ file already exists: " << pq_compressed_vectors_path
                              << ". Skipping PQ data generation." << std::endl;
                return 0; // 返回成功状态
            }
            else
            {
                diskann::cout << "Compressed PQ file exists but does not match the required chunks. "
                              << "Regenerating PQ data." << std::endl;
            }
        }
    }
    ...
    for (size_t block = 0; block < num_blocks; block++)
    {
        ...
        // 后续直接用 block_data_float 做中心化、旋转等
